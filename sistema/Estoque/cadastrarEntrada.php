...
echo "Quantidade de Linhas retornadas da Entrada de Mercadoria: ".$quantidadeLinhas."<br><br>";
...
echo "Usuário: ".$usuario. " - Data de Lançamento: ".$v_dataLancamento." - Código do Fornecedor: ".$v_codigoFornecedor." - Data de Vencimento: ".$v_dataVencimento." - Depósito: ".$v_deposito." - Parcelas: ".$v_parcelas."<br><br>";
...
for ($i=1; $i <= $quantidadeLinhas; $i++) { 
	...
	if (empty($produto[$i])) {
		...
		$tabelaOITM = "SELECT * FROM OITM WHERE idProduto = '$produto[$i]' AND cancelado = 'N' AND itemCompra = 'Y';";
		$resultadoOITM = mysqli_query($conexao, $tabelaOITM);
		$linhaOITM = mysqli_fetch_assoc($resultadoOITM);
		if (empty($linhaOITM['idProduto'])) {
			echo "Código de item inválido, CANCELADO ou não é item de Compra. Por favor refaça o procedimento anterior";
			return 0;
		}else{
...
echo "Valor Parcela: ".$valorParcela."<br><br>";
...
//INSERINDO OS DADOS NA TABELA OPCH
$insertOPCH = "INSERT INTO opch(idParceiroNegocio, idDeposito, dataLancamento, dataCadastro, dataVencimento, status, parcelas, valorTotal, usuarioCadastro, cancelado) VALUES('$v_codigoFornecedor', '$v_deposito', '$v_dataLancamento', '$data', '$v_dataVencimento', 'ABERTO', '$v_parcelas', '$totalNotaEntrada', '$usuario', 'N');";
$resultadoInsertOPCH = mysqli_query($conexao, $insertOPCH) or die('Erro ao cadastrar os dados na tabela OPCH');


$ultIdEntrada = mysqli_insert_id($conexao);

if (mysqli_affected_rows($conexao) <= 0) {
	echo "Erro ao tentar cadastrar Entrada de Mercadoria na tabela OPCH<br>";
	return 0;
}else{
	echo "Nota ".$ultIdEntrada." inserida com sucesso na tabela OPCH<br><br>";
}




//INSERINDO OS DADOS NA TABELA PCH1
for ($l=1; $l <= $numResultado; $l++) { 

	$insertPCH1 = "INSERT INTO pch1(idEntrada, numLinha, idProduto, quantidadeEntrada, idGrupoUnidadeMedida, valorUnitario, totalLinha, quantidadeConvertida, idDeposito, dataCadastro, usuarioCadastro, cancelado) VALUES('$ultIdEntrada', '$l','$v_CProduto[$l]', '$v_CQuantidade[$l]', '$v_CIdGUM[$l]', '$v_CValidado_valorUnitario[$l]', '$v_CValidado_valor[$l]', '$v_CQtdeTotal[$l]', '$v_deposito', '$data', '$usuario', 'N');";
	$resultadoInsertPCH1 = mysqli_query($conexao, $insertPCH1);

	if (mysqli_affected_rows($conexao) <= 0) {
		echo "A linha ".$l." não pode ser inserida na tabela PCH1 </br>";
		//CASO NÃO SEJA ADICIONADO OS DADOS NA TABELA PCH1
		$deleteOPCH = "DELETE FROM OPCH WHERE idEntrada = '$ultIdEntrada';";
		$resultadoDeleteOPCH = mysqli_query($conexao, $deleteOPCH);
		if (mysqli_affected_rows($conexao) > 0) {
			echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela OPCH<br>";
		}else{
			echo "IdEntrada: ".$ultIdEntrada." não foi excluído da tabela OPCH<br>";
		}

		return 0;
	}else{
		echo "A linha ".$l." foi inserida na tabela PCH1 com sucesso<br>";
	}
}
echo "<br>";
...
for ($k=1; $k <= $v_parcelas; $k++) { 
	$insertPCH2 = "INSERT INTO pch2(idEntrada, parcela, totalParcelas, valorParcela, status, dataVencParcela, dataCadastro, usuarioCadastro, cancelado) VALUES('$ultIdEntrada', '$k', '$v_parcelas', '$valorParcela', 'ABERTO', '$dataVencimentoParc[$k]', '$data', '$usuario', 'N');";
	$resultadoInsertPCH1 = mysqli_query($conexao, $insertPCH2);

	if (mysqli_affected_rows($conexao) <= 0) {
		echo "A linha ".$k." não pode ser inserida na tabela PCH2 </br>";
		//CASO NÃO SEJA ADICIONADO OS DADOS NA TABELA PCH2
		$deleteOPCH = "DELETE FROM OPCH WHERE idEntrada = '$ultIdEntrada';";
		$resultadoDeleteOPCH = mysqli_query($conexao, $deleteOPCH);
		if (mysqli_affected_rows($conexao) > 0) {
			echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela OPCH</br>";
		}else{
			echo "IdEntrada: ".$ultIdEntrada." não foi excluído da tabela OPCH";
		}

		$deletePCH1 = "DELETE FROM PCH1 WHERE idEntrada = '$ultIdEntrada';";
		$resultadoDeletePCH1 = mysqli_query($conexao, $deletePCH1);
		if (mysqli_affected_rows($conexao) > 0) {
			echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela PCH1</br>";
		}else{
			echo "IdEntrada: ".$ultIdEntrada." não foi excluído da tabela PCH1";
		}
		return 0;
	}else{
		echo "A linha ".$k." foi inserida na tabela PCH2 com sucesso<br>";
	}
}
...
for ($o=1; $o <= $numResultado; $o++) { 
	...
	if (mysqli_affected_rows($conexao) > 0) {
		...
		if (mysqli_affected_rows($conexao) <= 0) {
			...
			echo "Iniciando processo de restauração da tabela OPCH<br>";
			$deleteOPCH = "DELETE FROM OPCH WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeleteOPCH = mysqli_query($conexao, $deleteOPCH);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela OPCH<br>";
			...
			echo "Iniciando processo de restauração da tabela PCH1<br>";
			$deletePCH1 = "DELETE FROM PCH1 WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeletePCH1 = mysqli_query($conexao, $deletePCH1);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela PCH1</br>";
			...
			echo "Iniciando processo de restauração da tabela PCH2<br>";
			$deletePCH2 = "DELETE FROM PCH2 WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeletePCH2 = mysqli_query($conexao, $deletePCH2);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela PCH2</br>";
			}else{
				echo "IdEntrada: ".$ultIdEntrada." não foi excluído da tabela PCH2";
			}
		...
		$insertOITW = "INSERT INTO oitw(idProduto, idDeposito, quantidadeEstoque) VALUES('$v_CProduto[$o]', 1, '$v_CQtdeTotal[$o]');";
		$resultadoInsertOITW = mysqli_query($conexao, $insertOITW);
		if (mysqli_affected_rows($conexao) <= 0) {
			echo "Erro ao adicionar quantidade do item ".$v_CProduto[$o]." na tabela OITW";
			//CASO NÃO SEJA ADICIONADO OS DADOS NA TABELA OITW
			echo "<br>Iniciando processo de restauração da tabela OPCH<br>";
			$deleteOPCH = "DELETE FROM OPCH WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeleteOPCH = mysqli_query($conexao, $deleteOPCH);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela OPCH<br>";
			...
			echo "<br>Iniciando processo de restauração da tabela PCH1<br>";
			$deletePCH1 = "DELETE FROM PCH1 WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeletePCH1 = mysqli_query($conexao, $deletePCH1);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela PCH1</br>";
			...
			echo "<br>Iniciando processo de restauração da tabela PCH2<br>";
			$deletePCH2 = "DELETE FROM PCH2 WHERE idEntrada = '$ultIdEntrada';";
			$resultadoDeletePCH2 = mysqli_query($conexao, $deletePCH2);
			if (mysqli_affected_rows($conexao) > 0) {
				echo "IdEntrada: ".$ultIdEntrada." excluído com sucesso da tabela PCH2</br>";
