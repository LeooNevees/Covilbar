...
SELECT T2.idFicha, T2.nomeFicha, 'UN'
FROM OITT T2
WHERE T2.cancelado = 'N';";
...
//RETORNO DE TODA A TABELA OINV PARA CONSULTA
$tabelaOINV= "
SELECT T0.idSaida as 'idSaida', T1.nomeParceiro as 'nomeParceiro', date_format(T0.dataLancamento, '%d/%m/%Y') as 'dataLancamento', date_format(T0.dataVencimento, '%d/%m/%Y') as 'dataVencimento', T0.valorTotal as 'valorTotal'
FROM OINV T0
INNER JOIN OCRD T1
        ...
        <div class="tab-content" id="v-pills-tabContent">
            <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
                <div class="margem">
                    ...
                    <form class="needs-validation" method="POST" action="cadastrarSaida.php" novalidate target="_blank">
                        <div class="form-row">
            ...
            <div class="tab-pane fade alinhamentoTopParceiro" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
                <div class="margem2">
                    ...
                    <form class="needs-validation" method="POST" action="visualizarNota.php?tipoNota=2" target="_blank">
                        <div class="form-group">
                            <label for="inputAddress"><font color="yellow">Pesquisar Entrada de Mercadoria <font color="red">*</font></b></font></label>
                            <input type="text" class="form-control" id="idNota" name="idNota" placeholder="Busque a saida por Data ou Fornecedor" list="listaSaidaMercadoria" required>
                            <datalist id="listaSaidaMercadoria">
                                <?php while ($linhaConOINV = mysqli_fetch_assoc($resultadoOINV)) { ?>
                                    <option aria-disabled="true" value="<?php echo $linhaConOINV['idSaida'].' - '.$linhaConOINV['nomeParceiro'].' - '.$linhaConOINV['dataLancamento'].' - R$ '.number_format($linhaConOINV['valorTotal'], 2, ',', '.'); ?>" ></option>
                                <?php } ?>
                            </datalist>
                        </div><br>
